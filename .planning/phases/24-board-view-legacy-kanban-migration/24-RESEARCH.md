...
1. **Which of the two Pitfall-3 approaches should the plan implement — explicit second `createView` call, or PATCH the auto-created default view?**
   ...
   - Recommendation: Approach (a) — explicit second `createView({ type: 'board', ... })` — is simpler, has no failure mode tied to guessing the auto-view's shape, and the orphan row has zero UI/functional impact (Phase 25/26's "switch view type" UI will just show two views, one of which is blank/table with no records displayed differently — actually cosmetic risk is low since it's the same collection/records either way). Planner should pick one explicitly in the plan rather than leaving it ambiguous across tasks.

2. **Should the `'kanban'` slash-menu entry be removed/hidden once `collection-view` exists, or kept available for inserting new legacy boards?**
