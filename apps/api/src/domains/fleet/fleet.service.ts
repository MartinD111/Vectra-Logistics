...
class FleetService {
  ...
  async getDriver(driverId: string, companyId: string): Promise<Driver> {
    const d = await fleetRepository.findDriverById(driverId, companyId);
    if (!d) throw new AppError(404, 'Driver not found');
    return d;
  }

  async getVehicle(vehicleId: string, companyId: string): Promise<Vehicle> {
    const v = await fleetRepository.findVehicleById(vehicleId, companyId);
