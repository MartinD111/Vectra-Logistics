...
class FleetRepository {
  ...
  async findDriverById(driverId: string, companyId: string): Promise<Driver | null> {
    const { rows } = await db.query<Driver>(
      `SELECT * FROM drivers WHERE id = $1 AND company_id = $2 LIMIT 1`,
      [driverId, companyId],
    );
    return rows[0] ?? null;
  }

  async findVehicleById(vehicleId: string, companyId: string): Promise<Vehicle | null> {
    const { rows } = await db.query<Vehicle>(
      `SELECT * FROM vehicles WHERE id = $1 AND company_id = $2 LIMIT 1`,
      [vehicleId, companyId],
    );
    return rows[0] ?? null;
  }
