...
export const listCollections = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ collections: await recordsService.listCollections(requireCompany(req)) });
});

export const getCollection = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ collection: await recordsService.getCollection(req.params.id, requireCompany(req)) });
});
...
export const createCollection = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const { collection, view } = await recordsService.createCollection(requireCompany(req), req.body);
  res.status(201).json({ collection, view });
});

export const updateCollection = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ collection: await recordsService.updateCollection(req.params.id, requireCompany(req), req.body) });
});

// ── Records ──
export const listRecords = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ records: await recordsService.listRecords(req.params.id, requireCompany(req)) });
});

export const createRecord = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(201).json({ record: await recordsService.createRecord(requireCompany(req), req.body) });
});

export const getRecord = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ record: await recordsService.getRecord(req.params.id, requireCompany(req)) });
});

export const updateRecord = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ record: await recordsService.updateRecord(req.params.id, requireCompany(req), req.body) });
});

export const listRecordChildren = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ children: await recordsService.listRecordChildren(req.params.id, requireCompany(req)) });
});

// ── Views ──
export const listViews = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ views: await recordsService.listViews(req.params.id, requireCompany(req)) });
});

export const createView = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(201).json({ view: await recordsService.createView(requireCompany(req), req.params.id, req.body) });
});

export const getView = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ view: await recordsService.getView(req.params.id, requireCompany(req)) });
});

export const updateView = asyncHandler(async (req: AuthRequest, res: Response) => {
  res.status(200).json({ view: await recordsService.updateView(req.params.id, requireCompany(req), req.body) });
});
