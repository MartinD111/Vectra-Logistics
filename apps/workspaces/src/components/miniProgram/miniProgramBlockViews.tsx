...
// Per-kind player-side view components for mini-program blocks. Extracted from
// BlockView.tsx (now a thin registry-dispatch wrapper) so the mini-program block
// registry can point renderers directly at these components without a circular
// import. These read/write the shared runtime via useRuntime().
...
import type {
  FileInputBlock, PasteInputBlock, TableBlock, ExportBlock, CopyBlock,
...
export function FileInputView({ block }: { block: FileInputBlock }) {
  const rt = useRuntime();
...
export function PasteInputView({ block }: { block: PasteInputBlock }) {
  const rt = useRuntime();
  const rows = rt.state.inputs[block.id];
...
export function ExportView({ block }: { block: ExportBlock }) {
  const data = useDatasetFor(block.id);
...
export function CopyView({ block }: { block: CopyBlock }) {
  const data = useDatasetFor(block.id);
...
export function FormView({ block }: { block: FormBlock }) {
  const rt = useRuntime();
...
export function RecordsView({ block }: { block: RecordsBlock }) {
  const rt = useRuntime();
...
export function TsvOutputView({ block }: { block: TsvOutputBlock }) {
  const data = useDatasetFor(block.id);
...
export function DropdownView({ block }: { block: DropdownBlock }) {
  const rt = useRuntime();
