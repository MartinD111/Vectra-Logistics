...
class MarketplaceRepository {
  ...
  async findActiveShipments(): Promise<Shipment[]> {
    const { rows } = await db.query<Shipment>(
      `SELECT * FROM shipments WHERE status IN ('pending', 'open', 'booked', 'assigned', 'in_transit')
       ORDER BY created_at DESC`,
    );
    return rows;
  }

  async findCapacityById(capacityId: string): Promise<Capacity | null> {
    const { rows } = await db.query<Capacity>(
      `SELECT * FROM capacity_listings WHERE id = $1 LIMIT 1`,
      [capacityId],
    );
    return rows[0] ?? null;
  }

  async cancelShipment(shipmentId: string, userId: string): Promise<Shipment | null> {
    // Only the owner (creator) of the shipment can cancel it, and only when
  ...
  async cancelCapacity(capacityId: string, userId: string): Promise<Capacity | null> {
    const { rows } = await db.query<Capacity>(
